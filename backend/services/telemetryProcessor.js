...
function validateTelemetryPayload(payload) {
  ...
  if (
    typeof fuelLevelLitres !== 'number' ||
    fuelLevelLitres < 0
  ) {
    errors.push(
      'fuelLevelLitres must be a number >= 0'
    );
  }

  if (
    typeof fuelPercent !== 'number' ||
    fuelPercent < 0 ||
    fuelPercent > 100
  ) {
    errors.push(
      'fuelPercent must be between 0 and 100'
    );
  }
  ...
  if (
    engineOn !== undefined &&
    typeof engineOn !== 'boolean'
  ) {
    errors.push(
      'engineOn must be boolean when provided'
    );
  }
  ...
  if (
    ...
    errors.push(
      'speedLimitKmh must be > 0 when provided'
    );
  }
  ...
  if (
    ...
    (
      typeof loadWeightKg !== 'number' ||
      loadWeightKg < 0
    )
  ) {
    errors.push(
      'loadWeightKg must be >= 0 when provided'
    );
  }
  ...
  if (
    ...
    (
      typeof maxPayloadKg !== 'number' ||
      maxPayloadKg <= 0
    )
  ) {
    errors.push(
      'maxPayloadKg must be > 0 when provided'
    );
  }

  if (
    routeCompleted !== undefined &&
    typeof routeCompleted !== 'boolean'
  ) {
    errors.push(
      'routeCompleted must be boolean when provided'
    );
  }
...
async function processTelemetry(payload) {
  ...
  const telemetryRecord = {
    ...
    speedLimitKmh:
      payload.speedLimitKmh,
    loadWeightKg:
      payload.loadWeightKg,
    maxPayloadKg:
      payload.maxPayloadKg,
    routeCompleted:
